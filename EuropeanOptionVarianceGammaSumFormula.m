function [COP,POP,lo,mu] = EuropeanOptionVarianceGammaSumFormula(C, G, M, S, K, r ,q, tau, n1, m1,k1)
    ...
    COP =    real( K .* ((G*M).^(C*tau)) .* exp( -(r-q)*tau) .* (sum(sum(sum(Mat,5),4),3) + sum(sum(sum(Nat,5),4),3) + sum(sum(sum(Kat,5),4),3)) ./ ((gamma(C*tau)).^2));   % COP is defied as a two dimensional matrix that varies as K and tau vary by
