function [M2,Abs,Res,mu] = ConvergenceVG4D(C, G, M, S, K, r ,q, tau,  n1, m1,k1)
    ...
    for n = 0:n1
        for m = 0:m1
            for  k = 0:k1
                for t = 1:t1
                    for l = 1:r1
                        Nat(l,t,n+1,m+1,k+1) = ((-1)^(n+m)) * gamma(C*tau(t) + m) * gamma(1 + 2*C*tau(t) +k-n+m) * (M^(-1-2*C*tau(t)+n-k-m)) * (G^m) * (- lo(l,t))^n / (gamma(2 + C*tau(t) -n+k+m)*factorial(n)*factorial(m));
                    end
                end
            end
        end
            ...
            for  k = 0:k1
                for t = 1:t1
                    for l = 1:r1
                        ...
                        if (lo(l,t) > 0)
                            Kat(l,t,n+1,m+1,k+1) = ((-1)^(C*tau(t) + m)) * gamma(C*tau(t) + n) * gamma(C*tau(t) + m) * (M^n) * (G^m) * ((lo(l,t))^(1 + 2*C*tau(t) +k+m+n)) / (gamma(2 + 2*C*tau(t) +k+n+m)*factorial(n)*factorial(m));
                        else
    ...
    for n = 0:n1
        for m = 0:m1
            for  k = 0:k1   
                M2(:,:,n+1,m+1,k+1) = real( K .* ((G*M).^(C*tau)) .* exp( -(r-q)*tau) .* (Mat(:,:,n+1,m+1,k+1) + Nat(:,:,n+1,m+1,k+1) + Kat(:,:,n+1,m+1,k+1)) ./ ((gamma(C*tau)).^2));
                Res(n+1,m+1,k+1) = sqrt(sum(M2(:,:,n+1,m+1,k+1).^2,'all'));
                Abs(n+1,m+1,k+1) = sum(abs(M2(:,:,n+1,m+1,k+1)),'all');
            end
        end
    end
