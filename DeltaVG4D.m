function Delta = DeltaVG4D(C, G, M, S, K, r ,q, tau,  n1, m1,k1)
    ...
    Delta = real( K .* ((G*M).^(C*tau)) .* exp( -(r-q)*tau) .* (sum(sum(sum(Mat,5),4),3) + sum(sum(sum(Nat,5),4),3) + sum(sum(sum(Kat,5),4),3)) ./ (S*((gamma(C*tau)).^2)));    % Delta is defined as a two dimensional matrix, that varies as K and tau vary,
                                                                                                                                                                                % by summing the terms in Mat, Nat and Kat from 1 to n1, 1 to m1 and 1 to k1
end