function COP = CMVarianceGammaDerivative(C, G, M, S, K, r ,q, tau, n1, m1,k1,p)
    ...
    for t = 1:t1
        for l = 1:r1
            for n = 0:n1
                for m = 0:m1
                    for k = 0:k1
                        m0 = (tau(t)^2)*(log(G.*M)- 2 * Digamma(C.*tau(t),p))*(C/M) + tau(t)/M;
                        m11 = tau(t)*(Digamma(C*tau(t) + m,p) - 2*Digamma(-1 - 2*C*tau(t) -k-n-m,p)+ Digamma(1 - C*tau(t) -n,p) + 2*log(-lo(l,t)))*n/M;
        ...
        for l = 1:r1
            for n = 0:n1
                for m = 0:m1
                    for k = 0:k1
                        m0 = (tau(t)^2)*(log(G.*M)- 2 * Digamma(C.*tau(t),p))*(C/M) + tau(t)/M;
                        m22 = tau(t)*(Digamma(C*tau(t) + m,p) + 2*Digamma(1 + 2*C*tau(t) +k-n+m,p) - Digamma(2 + C*tau(t) -n+k+m,p) - 2*log(M))*n/M;
            ...
            for n = 0:n1
                for m = 0:m1
                    for k = 0:k1
                        if  (lo(l,t) > 0)
                            m0 = (tau(t)^2)*(log(G.*M)- 2 * Digamma(C.*tau(t),p))*(C/M) + tau(t)/M;
                            m33 = tau(t)*(Digamma(C*tau(t) + m,p) + Digamma(C*tau(t) + n,p) - 2* Digamma(2 + 2*C*tau(t) +k+n+m,p) + i*pi + 2*log(lo(l,t)))*n/M;
