function [CEC, CAC, CC, RhoT] = ConvergenceRhoVG(C, G, M, S, K, r ,q, tau,  n1)
    ...
    MatDelta = zeros(r1,t1,n1+1,n1+1,n1+1);
    ...
    for t = 1:t1
        for l = 1:r1
            for n = 0:n1
                for m = 0:n1
                    for k = 0:n1
                        MatDelta(l,t,n+1,m+1,k+1) = ((-1)^(n+m)) * gamma(C*tau(t) + m) * gamma(- 2*C*tau(t) -k-n-m) * (M^n) * (G^m) * (- lo(l,t) )^(2*C*tau(t) +k+n+m) / (gamma(1 - C*tau(t) -n)*factorial(n)*factorial(m)); % Mat(l,t,n+1,m+1,k+1) is defined as the value of the term in Delta^1_VG for the
                        Mat(l,t,n+1,m+1,k+1) = ((-1)^(n+m)) * gamma(C*tau(t) + m) * gamma(-1 - 2*C*tau(t) -k-n-m) * (M^n) * (G^m) * (- lo(l,t) )^(1 + 2*C*tau(t) +k+n+m) / (gamma(1 - C*tau(t) -n)*factorial(n)*factorial(m));  % Mat(l,t,n+1,m+1,k+1) is defined as the value of the term in C^1_VG for the
    ...
    NatDelta = zeros(r1,t1,n1+1,n1+1,n1+1);  % Nat is defined as zero five dimensional matrix
    Nat = zeros(r1,t1,n1+1,n1+1,n1+1);
    ...
    for n = 0:n1
        for m = 0:n1
            for  k = 0:n1
                for t = 1:t1
                    for l = 1:r1
                        NatDelta(l,t,n+1,m+1,k+1) = ((-1)^m) * gamma(C*tau(t) + m) * gamma( 2*C*tau(t) +k-n+m) * (M^(-2*C*tau(t)+n-k-m)) * (G^m) * (lo(l,t))^n / (gamma(1 + C*tau(t) -n+k+m)*factorial(n)*factorial(m));  % Nat(l,t,n+1,m+1,k+1) is defined as the value of the term in Delta^2_VG for the
                        Nat(l,t,n+1,m+1,k+1) = ((-1)^(n+m)) * gamma(C*tau(t) + m) * gamma(1 + 2*C*tau(t) +k-n+m) * (M^(-1-2*C*tau(t)+n-k-m)) * (G^m) * (- lo(l,t))^n / (gamma(2 + C*tau(t) -n+k+m)*factorial(n)*factorial(m));  % Nat(l,t,n+1,m+1,k+1) is defined as the value of the t
                    end                                                                                                                                                                                              % corresponding K(l), tau(t), n, m and k computed according to the Variance Gamma formula 
                end
            end
        end
    ...
    KatDelta = zeros(r1,t1,n1+1,n1+1,n1+1);  % Kat is defined as zero five dimensional matrix
    Kat = zeros(r1,t1,n1+1,n1+1,n1+1);
        ...
        for m = 0:n1
            for  k = 0:n1
                for t = 1:t1
                    for l = 1:r1
                        if (lo(l,t) > 0)
                            KatDelta(l,t,n+1,m+1,k+1) = ((-1)^(C*tau(t) + m)) * gamma(C*tau(t) + n) * gamma(C*tau(t) + m) * (M^n) * (G^m) * ((lo(l,t))^( 2*C*tau(t) +k+m+n)) / (gamma(1 + 2*C*tau(t) +k+n+m)*factorial(n)*factorial(m)); % if [log] > 0 Kat(l,t,n+1,m+1,k+1) is defined as the value of the term in Delta^3_VG for the
                            Kat(l,t,n+1,m+1,k+1) = ((-1)^(C*tau(t) + m)) * gamma(C*tau(t) + n) * gamma(C*tau(t) + m) * (M^n) * (G^m) * ((lo(l,t))^(1 + 2*C*tau(t) +k+m+n)) / (gamma(2 + 2*C*tau(t) +k+n+m)*factorial(n)*factorial(m));  % if [log] > 0 Kat(l,t,n+1,m+1,k+1) is defined as the value of the term in C^3_VG for the
                        else                                                                                                                                                                                                        % corresponding K(l), tau(t), n, m and k computed according to the Variance Gamma formula
                            KatDelta(l,t,n+1,m+1,k+1) = 0;   
    ...
    RhoT = real( K .* tau.* ((G*M).^(C*tau)) .* exp( -(r-q)*tau) .* (Mat - MatDelta + Nat - NatDelta + Kat - KatDelta) ./ (((gamma(C*tau)).^2)));    % Delta is defined as a two dimensional matrix, that varies as K and tau vary, by
