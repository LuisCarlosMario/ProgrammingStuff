function [T1,X,s1] = VarianceGammaRevenge(C, G, M, T, dt)
    ...
    [T1,G1,s1] = GammaProcessRevenge(C, M, T, dt);  % G1 is defined as the Gamma Process simulation matrix for each time value in vector T, for the parameters C, M
    [T2,G2,s2] = GammaProcessRevenge(C, G, T, dt);  % G2 is defined as the Gamma Process simulation matrix for each time value in vector T, for the parameters C, G
