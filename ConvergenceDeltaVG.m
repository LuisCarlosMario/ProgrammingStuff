function [CEC, CAC, CC, DeltaT] = ConvergenceDeltaVG(C, G, M, S, K, r ,q, tau,  n1)
    ...
    for t = 1:t1
        for l = 1:r1
            for n = 0:n1
                for m = 0:n1
                    for k = 0:n1
                        Mat(l,t,n+1,m+1,k+1) = ((-1)^(n+m)) * gamma(C*tau(t) + m) * gamma(- 2*C*tau(t) -k-n-m) * (M^n) * (G^m) * (- lo(l,t) )^(2*C*tau(t) +k+n+m) / (gamma(1 - C*tau(t) -n)*factorial(n)*factorial(m)); % Mat(l,t,n+1,m+1,k+1) is defined as the value of the term in Delta^1_VG for the
    ...
    for n = 0:n1
        for m = 0:n1
            for  k = 0:n1
                for t = 1:t1
                    for l = 1:r1
                        Nat(l,t,n+1,m+1,k+1) = ((-1)^m) * gamma(C*tau(t) + m) * gamma( 2*C*tau(t) +k-n+m) * (M^(-2*C*tau(t)+n-k-m)) * (G^m) * (lo(l,t))^n / (gamma(1 + C*tau(t) -n+k+m)*factorial(n)*factorial(m));  % Nat(l,t,n+1,m+1,k+1) is defined as the value of the term in Delta^2_VG for the
                    end                                                                                                                                                                                              % corresponding K(l), tau(t), n, m and k computed according to the Variance Gamma formula 
                end
            end
        end
        ...
        for m = 0:n1
            for  k = 0:n1
                for t = 1:t1
                    for l = 1:r1
                        if (lo(l,t) > 0)
                            Kat(l,t,n+1,m+1,k+1) = ((-1)^(C*tau(t) + m)) * gamma(C*tau(t) + n) * gamma(C*tau(t) + m) * (M^n) * (G^m) * ((lo(l,t))^( 2*C*tau(t) +k+m+n)) / (gamma(1 + 2*C*tau(t) +k+n+m)*factorial(n)*factorial(m)); % if [log] > 0 Kat(l,t,n+1,m+1,k+1) is defined as the value of the term in Delta^3_VG for the
    ...
    DeltaT = real( K .* ((G*M).^(C*tau)) .* exp( -(r-q)*tau) .* (Mat + Nat + Kat) ./ (S*((gamma(C*tau)).^2)));    % Delta is defined as a two dimensional matrix, that varies as K and tau vary, by
    ...
    for n = 1:n1+1
        for j = 1:n
            for k = 1:n
                CC(:,:,n,1) = CC(:,:,n,1) + DeltaT(:,:,n,j,k); % CC(:,:,:,1) is defined as the double sum series value matrix, for each K and tau element, when n is constant
                CC(:,:,n,2) = CC(:,:,n,2) + DeltaT(:,:,j,n,k); % CC(:,:,:,2) is defined as the double sum series value matrix, for each K and tau element, when m is constant
                CC(:,:,n,3) = CC(:,:,n,3) + DeltaT(:,:,j,k,n); % CC(:,:,:,3) is defined as the double sum series value matrix, for each K and tau element, when k is constant
            end
        end
    end
