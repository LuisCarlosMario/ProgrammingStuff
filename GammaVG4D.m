function Gamma = GammaVG4D(C, G, M, S, K, r ,q, tau,  n1, m1,k1)
    ...
    % Outputs:
    % Gamma - Ganna measure matrix of the European Call Option driven by the Variance Gamma process
    ...
    Gamma = real( K .* ((G*M).^(C*tau)) .* exp( -(r-q)*tau) .* (sum(sum(sum(Mat,5),4),3) + sum(sum(sum(Nat,5),4),3) + sum(sum(sum(Kat,5),4),3)) ./ ((S^2)*((gamma(C*tau)).^2)));    % Gamma is defined as a two dimensional matrix, that varies as K and tau vary,
