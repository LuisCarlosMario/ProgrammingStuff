function COP = CVarianceGammaDerivative(C, G, M, S, K, r ,q, tau, n1, m1,k1,p)
    ...
    for t = 1:t1
        for l = 1:r1
            for n = 0:n1
                for m = 0:m1
                    for k = 0:k1
                        m0 = (log(G*M)- 2 * Digamma(C*tau(t),p));
                        m11 = (             Digamma(C*tau(t) + m,p)-2*Digamma(-1 - 2*C*tau(t) -k-n-m,p)                                                     + Digamma(1 - C*tau(t) -n,p) + 2*log(-lo(l,t)));
                        MMat = ((-1)^(n+m)) * gamma(C*tau(t) + m) *     gamma(-1 - 2*C*tau(t) -k-n-m) * (M^n) * (G^m) * (- lo(l,t) )^(1 + 2*C*tau(t) +k+n+m) / (gamma(1 - C*tau(t) -n)*factorial(n)*factorial(m));  % Mat(l,t,n+1,m+1,k+1) is defined as the value of the term in C^1_VG for the
                        Mat(l,t,n+1,m+1,k+1) = tau(t)* (m0 + m11) * MMat;  % Mat(l,t,n+1,m+1,k+1) is defined as the value of the term in C^1_VG for the
        ...
        for l = 1:r1
            for n = 0:n1
                for m = 0:m1
                    for k = 0:k1
                        m0 = (log(G*M)- 2 * Digamma(C.*tau(t),p));
                        m22 = (             Digamma(C*tau(t) + m,p) + 2*Digamma(1 + 2*C*tau(t) +k-n+m,p)                                                  - Digamma(2 + C*tau(t) -n+k+m,p) - 2*log(M));
                        NNat = ((-1)^(n+m)) * gamma(C*tau(t) + m)       * gamma(1 + 2*C*tau(t) +k-n+m) * (M^(-1-2*C*tau(t)+n-k-m)) * (G^m) * (- lo(l,t))^n / (gamma(2 + C*tau(t) -n+k+m)*factorial(n)*factorial(m));  % Nat(l,t,n+1,m+1,k+1) is defined as the value of the term in C^2_VG for the
                        Nat(l,t,n+1,m+1,k+1) = tau(t)*(m0 + m22) * NNat;
                    end                                                                                                                                                                                                         % corresponding K(l), tau(t), n, m and k computed according to the Variance Gamma formula 
                end
            end
        end
            ...
            for n = 0:n1
                for m = 0:m1
                    for k = 0:k1
                        if  (lo(l,t) > 0)
                            m0 =(log(G.*M)- 2 .* Digamma(C.*tau(t),p));
                            m33 =                       (Digamma(C*tau(t) + n,p) + Digamma(C*tau(t) + m,p)                                                 - 2* Digamma(2 + 2*C*tau(t) +k+n+m,p) + i*pi +  2*log(lo(l,t)));
                            ...
                            Kat(l,t,n+1,m+1,k+1) =  tau(t).*(m0 + m33) * KKat;
