function COP = C2VarianceGammaDerivative(C, G, M, S, K, r ,q, tau, n1, m1,k1,p)
    ...
    for t = 1:t1
        for l = 1:r1
            for n = 0:n1
                for m = 0:m1
                    for k = 0:k1
                        m0 = ( (log(G.*M)- 2 * Digamma(C.*tau(t),p))^2 - 2 *Trigamma(C*tau(t),p));
                        m11 = (Digamma(C*tau(t) + m,p) - 2*Digamma(-1 - 2*C*tau(t) -k-n-m,p)+ Digamma(1 - C*tau(t) -n,p) + 2*log(-lo(l,t)))^2;
                        m12 = Trigamma(C*tau(t) + m,p) +4*Trigamma(-1 - 2*C*tau(t) -k-n-m,p)+Trigamma(1 - C*tau(t) -n,p);
                        ...
                        Mat(l,t,n+1,m+1,k+1) = (tau(t)^2)* (m0 + m11 + m12) * MMat;  % Mat(l,t,n+1,m+1,k+1) is defined as the value of the term in C^1_VG for the
        ...
        for l = 1:r1
            for n = 0:n1
                for m = 0:m1
                    for k = 0:k1
                        m0 = ( (log(G.*M)- 2 * Digamma(C.*tau(t),p))^2 - 2 *Trigamma(C*tau(t),p));
                        m21 = ((Digamma(C*tau(t) + m,p) + 2*Digamma(1 + 2*C*tau(t) +k-n+m,p) - Digamma(2 + C*tau(t) -n+k+m,p) - 2*log(M))^2);
                        m22 =  Trigamma(C*tau(t) + m,p) +4*Trigamma(1 + 2*C*tau(t) +k-n+m,p) -Trigamma(2 + C*tau(t) -n+k+m,p);
                        ...
                        Nat(l,t,n+1,m+1,k+1) = (tau(t)^2)*(m0 + m21 + m22) * NNat;
                    end                                                                                                                                                                                                         % corresponding K(l), tau(t), n, m and k computed according to the Variance Gamma formula 
                end
            end
        end
            ...
            for n = 0:n1
                for m = 0:m1
                    for k = 0:k1
                        if  (lo(l,t) > 0)
                            m0 = ( (log(G.*M)- 2 * Digamma(C.*tau(t),p))^2 - 2 *Trigamma(C*tau(t),p));
                            m31 = ((Digamma(C*tau(t) + m,p) + Digamma(C*tau(t) + n,p) - 2* Digamma(2 + 2*C*tau(t) +k+n+m,p) + i*pi + 2*log(lo(l,t)))^2);
                            m32 =  Trigamma(C*tau(t) + m,p) + Trigamma(C*tau(t) + n,p) - 4* Digamma(2 + 2*C*tau(t) +k+n+m,p);
                            ...
                            Kat(l,t,n+1,m+1,k+1) =  (tau(t)^2)*(m0 + m31 + m32) * KKat;
