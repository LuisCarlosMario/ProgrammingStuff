function [X,TM,S,s,m] = StockPriceVGRevenge(C, G, M, S0, r ,q, T, dt)
    ...
    [TM,X,s] = VarianceGammaRevenge(C, G, M, T, dt);        % X is defined as the matrix of Variance Gamma process simulations matrix for each time value in vector T
    m = CharacteristicFunctionVGRevenge(-i, C, G, M, TM);   % m is defined as the matrix of characteristic function values under a Variance Gamma process
